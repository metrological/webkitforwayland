...
private:
    ...
    template<typename Malloc> StringImpl(MallocPtr<Latin1Character, Malloc>, unsigned length);
    template<typename Malloc> StringImpl(MallocPtr<char16_t, Malloc>, unsigned length);
...
public:
    ...
    WTF_EXPORT_PRIVATE static Ref<StringImpl> createUninitialized(size_t length, Latin1Character*&);
    WTF_EXPORT_PRIVATE static Ref<StringImpl> createUninitialized(size_t length, char16_t*&);
    ...
    ALWAYS_INLINE std::span<const Latin1Character> span8() const LIFETIME_BOUND { ASSERT(is8Bit()); return { m_data8, length() }; }
    ALWAYS_INLINE std::span<const char16_t> span16() const LIFETIME_BOUND { ASSERT(!is8Bit() || isEmpty()); return { m_data16, length() }; }
    ...
    template<typename Func>
    static Expected<std::invoke_result_t<Func, std::span<const char8_t>>, UTF8ConversionError> tryGetUTF8ForCharacters(const Func&, std::span<const char16_t> characters, ConversionMode = LenientConversion);
...
public:
    ...
    ALWAYS_INLINE static void copyCharacters(char16_t* destination, std::span<const Latin1Character> source)
    {
    ...
    ALWAYS_INLINE static void copyCharacters(Latin1Character* destination, std::span<const char16_t> source)
    {
        static_assert(sizeof(char16_t) == sizeof(uint16_t));
        static_assert(sizeof(Latin1Character) == sizeof(uint8_t));
#if ASSERT_ENABLED
...
WTF_EXPORT_PRIVATE bool equal(const StringImpl*, const Latin1Character*);
inline bool equal(const StringImpl* a, const char* b) { return equal(a, byteCast<Latin1Character>(b)); }
...
inline bool equal(const Latin1Character* a, StringImpl* b) { return equal(b, a); }
inline bool equal(const char* a, StringImpl* b) { return equal(b, byteCast<Latin1Character>(a)); }
...
inline StringImpl::StringImpl(unsigned length, Force8Bit)
    : StringImplShape(s_refCountIncrement, { tailPointer<Latin1Character>(), length }, s_hashFlag8BitBuffer | StringNormal | BufferInternal)
{
    ASSERT(m_data8);
    ASSERT(m_length);
...
inline StringImpl::StringImpl(unsigned length)
    : StringImplShape(s_refCountIncrement, { tailPointer<char16_t>(), length }, s_hashZeroValue | StringNormal | BufferInternal)
{
    ASSERT(m_data16);
    ASSERT(m_length);
...
inline StringImpl::StringImpl(MallocPtr<Latin1Character, Malloc> characters, unsigned length)
    : StringImplShape(s_refCountIncrement, { static_cast<const Latin1Character*>(nullptr), length }, s_hashFlag8BitBuffer | StringNormal | BufferOwned)
{
    if constexpr (std::is_same_v<Malloc, StringImplMalloc>)
        m_data8 = characters.leakPtr();
    else {
        auto data8 = static_cast<Latin1Character*>(StringImplMalloc::malloc(length * sizeof(Latin1Character)));
...
inline StringImpl::StringImpl(MallocSpan<Latin1Character, Malloc> characters)
    : StringImplShape(s_refCountIncrement, { static_cast<const Latin1Character*>(nullptr), characters.span().size() }, s_hashFlag8BitBuffer | StringNormal | BufferOwned)
{
    if constexpr (std::is_same_v<Malloc, StringImplMalloc>)
        m_data8 = characters.leakSpan().data();
    else {
        auto* data8 = static_cast<Latin1Character*>(StringImplMalloc::malloc(characters.sizeInBytes()));
...
template<typename Malloc>
inline StringImpl::StringImpl(MallocPtr<char16_t, Malloc> characters, unsigned length)
    : StringImplShape(s_refCountIncrement, { static_cast<const char16_t*>(nullptr), length }, s_hashZeroValue | StringNormal | BufferOwned)
{
    if constexpr (std::is_same_v<Malloc, StringImplMalloc>)
        m_data16 = characters.leakPtr();
    else {
        auto data16 = static_cast<char16_t*>(StringImplMalloc::malloc(length * sizeof(char16_t)));
...
inline StringImpl::StringImpl(MallocSpan<char16_t, Malloc> characters)
    : StringImplShape(s_refCountIncrement, { static_cast<const char16_t*>(nullptr), characters.span().size() }, s_hashZeroValue | StringNormal | BufferOwned)
{
    if constexpr (std::is_same_v<Malloc, StringImplMalloc>)
        m_data16 = characters.leakSpan().data();
    else {
        auto* data16 = static_cast<char16_t*>(StringImplMalloc::malloc(characters.sizeInBytes()));
...
template<typename Func>
inline Expected<std::invoke_result_t<Func, std::span<const char8_t>>, UTF8ConversionError> StringImpl::tryGetUTF8ForCharacters(const Func& function, std::span<const char16_t> characters, ConversionMode mode)
{
    if (characters.empty())
        return function(nonNullEmptyUTF8Span());
