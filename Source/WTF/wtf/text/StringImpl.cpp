...
template Ref<StringImpl> StringImpl::createUninitializedInternalNonEmpty(size_t length, Latin1Character*& data);
template Ref<StringImpl> StringImpl::createUninitializedInternalNonEmpty(size_t length, char16_t*& data);

Ref<StringImpl> StringImpl::createUninitialized(size_t length, Latin1Character*& data)
{
    return createUninitializedInternal(length, data);
}

Ref<StringImpl> StringImpl::createUninitialized(size_t length, char16_t*& data)
{
    return createUninitializedInternal(length, data);
}
...
Ref<StringImpl> StringImpl::convertToLowercaseWithoutLocale()
{
    ...
    if (is8Bit()) {
        for (unsigned i = 0; i < m_length; ++i) {
            Latin1Character character = m_data8[i];
...
ALWAYS_INLINE static bool equalInner(const StringImpl& string, unsigned start, std::span<const char> matchString)
{
    ...
    if (string.is8Bit())
        return equal(string.span8().data() + start, byteCast<Latin1Character>(matchString));
    return equal(string.span16().data() + start, byteCast<Latin1Character>(matchString));
}
...
bool equal(const StringImpl* a, const Latin1Character* b)
{
    if (!a)
        return !b;
    if (!b)
    ...
    if (a->is8Bit()) {
        ...
        for (unsigned i = 0; i != length; ++i) {
            Latin1Character bc = b[i];
            Latin1Character ac = aSpan[i];
            if (!bc)
                return false;
            if (ac != bc)
                return false;
