...
public:
    ...
    std::span<const Latin1Character> span8() const { ASSERT(is8Bit()); return m_text.span8(); }
    std::span<const char16_t> span16() const { ASSERT(!is8Bit()); return m_text.span16(); }
    std::span<const Latin1Character> subspan8(unsigned i) const { return span8().subspan(i); }
    std::span<const char16_t> subspan16(unsigned i) const { return span16().subspan(i); }
