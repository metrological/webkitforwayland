...
void AppendPipeline::appsinkNewSample(Track& track, GRefPtr<GstSample>&& sample)
{
    ...
    if (segment && (segment->time || segment->start)) {
        ...
        && (track.extendedFirstSampleOffsetTimestamp.isInvalid() || track.extendedFirstSampleOffsetTimestamp != m_sourceBufferPrivate.timestampOffset())) {
        ...
        track.extendedFirstSampleOffsetTimestamp = m_sourceBufferPrivate.timestampOffset();
    }
...
void AppendPipeline::resetParserState()
{
    ...
    for (std::unique_ptr<Track>& track : m_tracks)
        track->extendedFirstSampleOffsetTimestamp = MediaTime::invalidTime();
