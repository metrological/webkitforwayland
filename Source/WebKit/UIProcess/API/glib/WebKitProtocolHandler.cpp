...
void WebKitProtocolHandler::handleGPU(WebKitURISchemeRequest* request)
{
    ...
    addTableRow(versionObject, "WebKit version"_s, makeString(webkitPortName(), ' ', WEBKIT_MAJOR_VERSION, '.', WEBKIT_MINOR_VERSION, '.', WEBKIT_MICRO_VERSION, " ("_s, WTF::unsafeSpan(BUILD_REVISION), ')'));
...
#if USE(CAIRO)
    addTableRow(versionObject, "Cairo version"_s, makeString(WTF::unsafeSpan(CAIRO_VERSION_STRING), " (build) "_s, WTF::unsafeSpan(cairo_version_string()), " (runtime)"_s));
#endif
